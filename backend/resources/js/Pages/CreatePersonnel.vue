...
const submitForm = () => {
  if (!nom.value || !prenom.value || !directionId.value || !service.value || !grade.value || !corp.value || !fonction.value || !email.value || !photo.value) {
    Swal.fire({
      icon: 'warning',
  ...
  router.post(route('personnel.store'), formData, {
    ...
    onError: (errors) => {
      Swal.fire({
        icon: 'error',
        title: 'Erreur',
        text: errors.response?.data?.message || 'Une erreur est survenue lors de l\'ajout du personnel.',
      });
    },
  });
};
  ...
  <AuthenticatedLayout>
    ...
    <div class="container">
      <form @submit.prevent="submitForm" enctype="multipart/form-data">
        ...
        <div class="row mt-3">
          <div class="col">
            <button type="submit" class="btn btn-primary w-100">Ajouter Personnel</button>
          </div>
        </div>
      </form>
    </div>
