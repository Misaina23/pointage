...
export const SecurityAbsenceScreen = () => {
  ...
  const fetchAbsences = async () => {
    ...
    setLoading(true);
    try {
      const response = await securityApi.getMyAbsences({ per_page: 50 });
  ...
  const handleCreateAbsence = async () => {
    ...
    try {
      ...
      await securityApi.createAbsence(data);
  ...
  const handleDateChange = (event: any, selectedDate: Date | undefined) => {
    if (selectedDate && showDatePicker) {
      setFormData({ ...formData, [showDatePicker]: selectedDate });
    }
    setShowDatePicker(null);
  };

  const openDatePicker = (fieldName: 'date_debut' | 'date_fin') => {
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={[styles.header, { borderBottomColor: colors.border }]}>
        ...
        <Text style={[styles.headerTitle, { color: colors.text }]}>Mes demandes d'absence</Text>
        <View style={{ width: 80 }} />
      </View>
      ...
      <ScrollView style={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {personnel && (
          <View style={styles.securityInfoBanner}>
            <Text style={[styles.securityInfo, { color: colors.textSecondary }]}>
              Connecté en tant que sécurité: {personnel.prenom} {personnel.nom}
            </Text>
          </View>
        )}
        ...
        >
          <Text style={styles.createButtonText}>+ Nouvelle demande d'absence</Text>
        </TouchableOpacity>
